    ...
    for (int x = 0, y = 0; std::getline(file, line); y++) {
        for (x = 0; x < (int) line.size(); x++) {
            if (line[x] == '#') {
                ...
                _module->setObject(_data, "exit_" + std::to_string(_exitNb), GT_PATH "pacman/exit.png", TT_PATH "pacman/exit", x + xOrigin, y + yOrigin); 
                _exitNb++;
            }*/
        }
    }
...
int PacmanGame::coreGame()
{
    ...
    /*
    for (int i = 0; i < _exitLeftNb; i++) {
        _module->getObjectCoordinates(_data, "exit_" + std::to_string(i), xObject_1, yObject_1);
        if (xPlayer + _xDirection == xObject_1 && yPlayer + _yDirection == yObject_1) {
            _module->getObjectCoordinates(_data, "exit_" + std::to_string(i - 1), )
        }
    }*/
    if (!collision)
