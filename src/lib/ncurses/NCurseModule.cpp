...
void NCurseModule::setObjectTexture(std::map<std::string, Object> &gameDatas, const std::string &name, const std::string &sfTexturePath, const std::string &ncTexturePath)
{
    std::map<std::string, Object>::iterator it = gameDatas.find(name);
...
int NCurseModule::getInputs() const
{
    return getch();
}